...
const CreateMainWindow = () => {
  ...
  configdata = fs.readFileSync(path.join(configdata, 'config.conf'), {
    encoding: 'utf8',
    flag: 'r',
  });
  if (configdata) {
  ...
  fs.readdir(filePath, function (err, files) {
    ...
    files.forEach((filename) => {
      ...
      if (fs.statSync(fullpath).isFile() && filename != 'main.js') {
        let fileObj: any = loadFile(fullpath);
        const fns = {};
        //加载函数
        ...
        for (const key in fns) {
          ...
          ipcMain.on(channel, async (event, params) => {
            try {
              ...
              let itemfileObj: any = loadFile(itemfullpath);
...
function loadFile(filepath) {
  ...
  try {
    ...
    // require js module
    const obj: any = require(filepath);
    if (!obj) return obj;
    // it's es module
